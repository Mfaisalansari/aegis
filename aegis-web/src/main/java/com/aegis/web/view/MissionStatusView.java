...
/** Renders {@code GET /missions/{id}} — a live action stream while RUNNING, a timeline-style summary once DONE/ERROR. */
    ...
    public static String render(MissionJob job) {
        ...
        switch (job.state()) {
            case RUNNING -> {
                ...
                body.append(LiveMissionStreamView.render(job));
            }
            case ERROR -> {
            ...
            default -> {
                ...
                String statusClass = summary.status().name().toLowerCase(Locale.ROOT);
                ...
                body.append("<div class=\"stat-row\"><span class=\"k\">Status</span><span class=\"v\">").append(summary.status()).append("</span></div>");
        ...
        return Layout.page("AEGIS — Mission " + job.id(), body.toString());
    }
}
